<template>
  <div class="h-full flex flex-col clay-card overflow-hidden">
    <div class="p-4 bg-primary text-white">
      <h2 class="font-sans text-xl font-bold flex items-center gap-2">
      ...
      <div class="flex gap-2">
        <div class="relative flex-1">
          <input 
            ...
            maxlength="20"
            class="w-full clay-input pr-12 font-bold text-text-color"
            placeholder="输入昵称..."
          />
        ...
        <button 
          @click="saveName"
          class="clay-button bg-cta p-2 shrink-0"
    ...
    <div class="flex-1 overflow-y-auto p-2 custom-scrollbar">
      <TransitionGroup name="user-list" tag="ul" class="space-y-2">
        <li 
          v-for="user in users" 
          :key="user.id"
          class="flex items-center gap-3 p-3 rounded-clay hover:shadow-clay-sm hover:scale-[1.02] transition-all duration-200 bg-white"
          :class="{ 'border-2 border-primary/20': user.id === currentUserId }"
        >
          <div 
            class="w-10 h-10 rounded-clay flex items-center justify-center text-white font-bold text-lg shadow-inner"
          ...
          <div v-if="user.id === currentUserId" class="w-2 h-2 bg-cta rounded-full shadow-lg"></div>
        </li>
      </TransitionGroup>
    </div>
  </div>
